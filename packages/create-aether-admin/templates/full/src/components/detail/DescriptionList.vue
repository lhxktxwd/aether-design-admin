...
<template>
  <dl class="grid overflow-hidden rounded-admin border border-admin-border-soft bg-admin-surface" :class="columns === 3 ? 'md:grid-cols-3' : columns === 2 ? 'md:grid-cols-2' : 'grid-cols-1'">
    <div v-for="item in items" :key="item.label" class="border-b border-admin-border-soft p-4 md:border-r">
      ...
      <dd class="mt-2 flex min-h-6 items-center gap-2 text-sm font-[650] text-admin-text">
        ...
        <ElButton v-if="item.copyable && item.value" :icon="DocumentCopy" text circle size="small" :aria-label="`Copy ${item.label}`" @click="copyValue(item.value)" />
      </dd>
    </div>
  </dl>
</template>
