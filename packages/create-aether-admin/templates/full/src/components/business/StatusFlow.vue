...
const currentIndex = computed(() => Math.max(0, props.steps.findIndex((step) => step.key === current.value)));
    ...
    <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
      <div class="flex min-w-0 flex-1 flex-wrap items-center gap-2">
        <button
          ...
          :class="index === currentIndex ? 'border-admin-primary bg-admin-primary-soft' : 'border-admin-border-soft bg-admin-surface-2 hover:border-admin-border'"
          :disabled="disabled"
          @click="moveTo(index)"
        >
          <span class="flex size-6 shrink-0 items-center justify-center rounded-full text-xs font-bold" :class="index <= currentIndex ? 'bg-admin-primary text-white' : 'bg-admin-surface text-admin-muted'">
            {{ index + 1 }}
          </span>
          <span class="min-w-0">
      ...
      <div class="flex shrink-0 items-center gap-2">
        <AetherStatusTag :label="steps[currentIndex]?.label ?? current" :tone="steps[currentIndex]?.tone ?? 'primary'" />
