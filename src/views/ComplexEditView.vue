...
<template>
  <section>
    <PageHeader
      eyebrow="Complex edit"
      title="Complex Edit Page"
      description="Long form navigation, grouped sections, dirty state and sticky footer actions."
      :breadcrumbs="['Admin', 'Complex Edit']"
    />
      ...
      <FormSection title="Profile" description="Core identity and ownership fields." :columns="2">
        ...
        <ElFormItem label="Region"
          ><ElSelect v-model="form.region" class="w-full"
            ><ElOption label="Singapore" value="Singapore" /><ElOption label="Virginia" value="Virginia" /></ElSelect
        ></ElFormItem>
      </FormSection>
      <FormSection
        title="Permissions"
        description="Advanced settings can open a drawer for focused editing."
        :columns="1"
      >
        <ElFormItem label="API access" required
          ><ElSelect v-model="form.apiAccess" class="w-full"
            ><ElOption label="Enabled" value="enabled" /><ElOption label="Disabled" value="disabled" /></ElSelect
        ></ElFormItem>
        <button type="button" class="text-left text-sm font-650 text-admin-primary" @click="drawerOpen = true">
    ...
    <FormDrawer
      v-model="drawerOpen"
      title="Permission drawer"
      description="Focused drawer for nested permission editing."
      :dirty="dirty"
