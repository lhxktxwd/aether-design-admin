...
<template>
  ...
  >
    ...
    <ElForm label-position="top" class="space-y-1" :disabled="disabled || loading">
      <div class="grid gap-4 lg:grid-cols-3">
        ...
        <ElFormItem label="Tags">
          <ElSelect v-model="filters.tags" multiple collapse-tags collapse-tags-tooltip clearable class="w-full" placeholder="Select tags">
            <ElOption v-for="option in tagOptions" :key="option.value" :label="option.label" :value="option.value" :disabled="option.disabled" />
          </ElSelect>
        </ElFormItem>

        <ElFormItem label="Owner">
          <ElSelect v-model="filters.owner" clearable class="w-full" placeholder="Any owner">
            <ElOption v-for="option in ownerOptions" :key="option.value" :label="option.label" :value="option.value" :disabled="option.disabled" />
          </ElSelect>
        </ElFormItem>

        <ElFormItem label="Region">
          <ElSelect v-model="filters.region" clearable class="w-full" placeholder="Any region">
            <ElOption v-for="option in regionOptions" :key="option.value" :label="option.label" :value="option.value" :disabled="option.disabled" />
          </ElSelect>
        </ElFormItem>
      </div>

      <div class="grid gap-4 lg:grid-cols-2">
        <ElFormItem :error="amountInvalid ? 'Minimum amount cannot be greater than maximum amount.' : ''" label="Amount range">
          <div class="grid w-full grid-cols-2 gap-3">
            <ElInputNumber v-model="filters.minAmount" class="!w-full" :min="0" placeholder="Min" controls-position="right" />
            <ElInputNumber v-model="filters.maxAmount" class="!w-full" :min="0" placeholder="Max" controls-position="right" />
          </div>
        </ElFormItem>

        <ElFormItem label="Selected tags">
          <div class="flex min-h-8 flex-wrap items-center gap-2 rounded-admin-sm border border-admin-border-soft bg-admin-surface-2 px-3 py-2">
    ...
    <footer class="mt-4 flex flex-col gap-3 border-t border-admin-border-soft pt-4 md:flex-row md:items-center md:justify-between">
      ...
      <div class="flex shrink-0 items-center gap-2">
        ...
        <ElButton type="primary" :loading="loading" :disabled="disabled || amountInvalid" @click="emit('apply', filters)">
          Apply filters
        </ElButton>
      </div>
    </footer>
